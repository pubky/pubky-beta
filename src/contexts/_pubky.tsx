...
export function PubkyClientWrapper({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    pubky && setSpecsBuilder(new PubkySpecsBuilder(pubky));
  }, [pubky]);
  ...
  const isSessionActive = async () => {
    try {
      ...
      return { status: true, message: error || 'homeserver down' };
    }
  };
  ...
  const createPost = withAuth(
    async (
      ...
      const newPostDetails: PostDetails = {
        ...
        kind: result.details.kind as unknown as PubkyAppPostKind,
