...
export default function ShowAllTags({ post, postType, onTagClick }: ShowAllTagsProps) {
  ...
  useEffect(() => {
    setSkip(limit);
    setHasMore(post.counts?.tags > limit);
    // Sort tags by taggers_count in descending order
    setAllTags([...tags].sort((a, b) => (b.taggers_count || 0) - (a.taggers_count || 0)));
  }, [post.details.id]);

  // Update allTags when tags change
  useEffect(() => {
    setAllTags((prev) => {
      const newTags = tags.filter((tag) => !prev.some((t) => t.label === tag.label));
      // Keep new tags at the top, don't sort them
      return [...newTags, ...prev];
    });
  }, [tags]);
  ...
  useEffect(() => {
    if (!isLoading && moreTags) {
      setAllTags((prev) => {
        ...
        // Sort all tags by taggers_count
  ...
  // Use infinite scroll for taggers
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        const [entry] = entries;
  ...
  const handleAddTagInput = async (tag: string) => {
    ...
    try {
      ...
      // Add optimistic tag at the top of the list
