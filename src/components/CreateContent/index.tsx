    ...
    if (items) {
      ...
      if (filesToProcess.length > 0) {
        ...
        for (let i = 0; i < filesToProcess.length; i++) {
          ...
          // Try to convert unsupported video types to MP4 first
          if (isVideo && !Utils.supportedVideoTypes.includes(file.type)) {
            ...
            try {
              ...
              loadingAlertId = addAlert(
                `Converting video ${i + 1}/${filesToProcess.length} to MP4... 0%`,
                'loading',
                () => {
              ...
              const convertedFile = await Utils.convertVideo(
                ...
                (progress) => {
                  ...
                  if (loadingAlertId) {
                    updateAlert(
                      loadingAlertId,
                      `Converting video ${i + 1}/${filesToProcess.length} to MP4... ${progress}%`
                    );
                  }
              ...
              if (convertedFile.size > maxOtherSizeInBytes) {
                ...
                try {
                  ...
                  compressionLoadingAlertId = addAlert(
                    `Compressing converted video ${i + 1}/${filesToProcess.length}...`,
                    'loading',
                    () => {
                  ...
                  const resizedFile = await Utils.resizeVideoFile(
                    ...
                    (progress) => {
                      ...
                      updateAlert(
                        ...
                        `Compressing converted video ${i + 1}/${filesToProcess.length}... ${progress}%`
                      );
