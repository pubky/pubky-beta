...
export const handleAddTag = async (
  ...
  // check if the tag exists and the user is already a tagger
  const existingTag = localPost.tags.find((t) => t.label === tag);
  if (existingTag && existingTag.taggers.includes(pubky)) {
    addAlert('You have already added this tag', 'warning');
    return;
  }
