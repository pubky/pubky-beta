...
import { SimplePostTags } from '@/components';
      ...
      <PostUI.MainCard className={restClassName}>
        <div>
          ...
          <div className={`flex flex-col md:flex-row ${largeView ? 'gap-2' : 'justify-between'}`}>
            {!repostView && <SimplePostTags.PostStandard largeView={largeView} post={repostedPost} />}
            {!repostView && <PostComponents.Actions post={repostedPost} />}
          </div>
        </div>
        {largeView && <SimplePostTags.PostLargeView post={repostedPost} />}
      </PostUI.MainCard>
    </div>
  );
}
