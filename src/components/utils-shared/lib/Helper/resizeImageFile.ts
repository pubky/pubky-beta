...
// Utility to resize an image file and return a data URL string (for markdown editors)
export async function resizeImageFileToDataURL(file: File, maxSizeInBytes: number = 5 * 1024 * 1024): Promise<string> {
  return new Promise((resolve, reject) => {
    const img = new window.Image();
    const reader = new FileReader();
    ...
    img.onload = () => {
      ...
      const resizeAndCompress = () => {
        ...
        // Check if file size is within limit by converting to blob temporarily
        canvas.toBlob(
          (blob) => {
            if (!blob) return reject('Failed to create blob');
