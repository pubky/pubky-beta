...
export default function Tags({
  ...
  const [selectedTag, setSelectedTag] = useState<ITaggedProfile>(
    tagsProfile[0]
  );
  ...
  const handleTagClick = (tag: ITaggedProfile) => {
    setSelectedTag(tag);
  };
  ...
  useEffect(() => {
    if (tagsProfile?.length === 0) {
      setShowModalTags(false);
    }
  }, [tagsProfile, setShowModalTags]);
  ...
  useEffect(() => {
    async function fetchFollowing() {
      try {
        ...
        if (following) {
          ...
          const matchedFollowedIds = tagsProfile
    ...
    fetchFollowing();
  }, [pubky, listFollowing, tagsProfile]);
